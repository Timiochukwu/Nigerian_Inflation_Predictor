...
The `print` lines are just temporary -- they confirm the file loads without errors. We will delete them in the next step.

---

### Step 2: Add the First Function -- `enforce_monthly_frequency`

Now **delete the two `print(...)` lines** at the bottom and replace them with the first real function and a test block.
...
Add this function **above** the `if __name__` block (not inside it):
...
Now **update the `if __name__` block** -- add two lines after the frequency step:
...
**Update the `if __name__` block** -- add at the bottom:
...
Now we tie everything together. Add these two functions **above** the `if __name__` block:
...
Now **replace the entire `if __name__` block** with this:
...
Here is the entire file as it should look after all 5 steps. Compare your file against this to make sure nothing is missing or out of order.
